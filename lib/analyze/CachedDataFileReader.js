...
function createCachedDataFileReader(dataFileDir) {
	...
	return { getDataFileDir, clearAllCache, clearCache, prepareFiles, readFile };

	function getDataFileDir() {
		return dataFileDir;
	}
	...
	function prepareFiles(fileList) {
		...
		const absPath = fileList.map(file => path.join(dataFileDir, file));
		const prepare = []
			.concat(absPath.map(abs => fs.readFile(abs, 'utf8')))
			.concat(absPath.map(abs => fs.stat(abs)));
	...
	function readFile(fileName) {
		...
		const absPath = path.join(dataFileDir, fileName);
		return Promise.all([
