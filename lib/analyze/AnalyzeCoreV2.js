...
class AnalyzeCoreV2 {
	static get GroupBy() { return GROUP_BY; }
	...
	analyze(start, end) {
		...
		return fs.readdir(this.reader.getDataFileDir()).then(files => {
			/*
				Step 1: find all existed files in analyze range
			*/
			...
			/*
				Step 2: pre-analyze each file and get all data lines and projects
			*/
			...
			/*
				Step 3: analyze each data items
			*/
			...
			for (let i = 0; i < lineCount; i++) {
				...
				// This record is absolute file (means it is not located in current project)
				if (isEncodedPathAbsolute(fileName)) {
					// scan each project path
					for (const testProject of projectsArray) {
						if (fileName.startsWith(testProject)) { // maybe in other project
							const prefixLen = testProject.length;
							const inDir = fileName.slice(prefixLen, prefixLen + 3);
							// '%': { '%2F': '/', '%5C': '\\' }
							if (inDir == '%2F' || inDir == '%5C') {
								fileName = fileName.slice(testProject.length + 3);
								project = testProject;
							}
						} // </if>
					} // </for>
				} // </if> isEncodedPathAbsolute
