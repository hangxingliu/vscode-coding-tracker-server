...
const GROUP_BY_VCS = GroupBy.VCS_BRANCH | GroupBy.VCS_REPO;
...
function vcs(from, to, req, res) { return analyze(res, from, to, GROUP_BY_VCS); }
function project(from, to, project, req, res) {
	if (!project) return responseError(res, 400, 'param project is invalid!');
	return analyze(res, from, to, GROUP_BY_PROJECTS, {
		project: [project]
	});
}
...
function analyze(res, _from, _to, groupBy, filter) { 
	...
	analyze.analyze(new Date(from), new Date(to))
		? res.json(analyze.getResult())
		: responseError(res, 500, analyze.getError());
}
