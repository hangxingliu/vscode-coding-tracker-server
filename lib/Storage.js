...
module.exports = {
	...
	write: function (_data) {
		...
		getFileVersion(fname, version => {
			...
			try {
				if (version == null) {
					log.info(`new storage data file: ${fname}`);
				...
				//@ts-ignore
				global.DEBUG && debug(Path.basename(fname), _data);
			}catch (ex) { 
				error(ex);
			}
		});
...
function debug(fname, _data) { 
	log.success(`Stored success:  ${_data.type} (${_data.file}) ${getReadableLong(_data.long)}  => ${fname}`);
}

/** @param {number} ms */
function getReadableLong(ms) {
